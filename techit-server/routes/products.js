...
router.post("/", auth, async (req, res) => {
    try {
        ...
        if (!req.payload.isAdmin) return res.status(400).send("Access denied. User is not admin");
...
router.put("/:id", auth, async (req, res) => {
    try {
        ...
        //3. check if product already exists
        let product = await Product.findOneAndUpdate({ _id: req.params.id }, req.body, { new: true })//new true מחזיר את האובייקט אחרי העדכון
